...
function ShaderCoreALUActive()
{
    var rate = 0;
    if (MTLStatComputeCost != 0)
        rate = (((AMDStat_Compute_SALU_Inst_Cycles + AMDStat_Compute_VALU_Thread_Cycles / AverageThreadsPerComputeWavefront() ) / NumSE) / AMDStat_GPU_Engine_Busy_Ticks) / (NumCUPerSH * NumSH) * 100;
    else
        rate = (((AMDStat_Shader_SALU_Cycles + AMDStat_Vertex_VALU_Thread_Cycles / AverageThreadsPerVertexWavefront() + AMDStat_Fragment_VALU_Thread_Cycles / AverageThreadsPerFragmentWavefront()) / NumSE) / AMDStat_GPU_Engine_Busy_Ticks) / (NumCUPerSH * NumSH) * 100;
...
{
    var rate = (((AMDStat_Vertex_SALU_Inst_Cycles + AMDStat_Vertex_VALU_Thread_Cycles / AverageThreadsPerVertexWavefront()) / NumSE) / AMDStat_GPU_Engine_Busy_Ticks) / (NumCUPerSH * NumSH) * 100;
    return (rate > 100 ? 100 : rate);
}

function ShaderCoreFragmentALUActive()
{
    var rate = (((AMDStat_Fragment_SALU_Inst_Cycles + AMDStat_Fragment_VALU_Thread_Cycles / AverageThreadsPerFragmentWavefront()) / NumSE) / AMDStat_GPU_Engine_Busy_Ticks) / (NumCUPerSH * NumSH) * 100;
    return (rate > 100 ? 100 : rate);
}
...
function ShaderCoreVertexStall()
{
    ...
    var Active = (((AMDStat_Vertex_VALU_Thread_Cycles / AverageThreadsPerVertexWavefront() + AMDStat_Vertex_SALU_Inst_Cycles + AMDStat_Vertex_VMEM_Read_Cycles + AMDStat_Vertex_VMEM_Write_Cycles + AMDStat_Vertex_SMEM_Inst_Cycles + AMDStat_Vertex_EXP_Inst_Cycles) / NumSE) / AMDStat_GPU_Engine_Busy_Ticks) / (NumCUPerSH * NumSH) * 100;
...
{
    var Active = (((AMDStat_Fragment_VALU_Thread_Cycles / AverageThreadsPerFragmentWavefront() + AMDStat_Fragment_SALU_Inst_Cycles + AMDStat_Fragment_VMEM_Read_Cycles + AMDStat_Fragment_VMEM_Write_Cycles+ AMDStat_Fragment_SMEM_Inst_Cycles /*+ AMDStat_Fragment_EXP_Inst_Cycles*/) / NumSE) / AMDStat_GPU_Engine_Busy_Ticks) / (NumCUPerSH * NumSH) * 100;
...
{
    var allWriteReadOps = AMDStat_L1_Cache_Read_Request +
                          AMDStat_L1_Cache_Write_Request +
                          AMDStat_L1_Cache_atomic_w_ret_req +
                          AMDStat_L1_Cache_atomic_wo_ret_req;
    if( AMDStat_L1_Cache_Accesses != 0)
        hitRate = ((allWriteReadOps / AMDStat_L1_Cache_Accesses)) * 100;
    else
        hitRate = 0;
...
{
    return AMDStat_L1_Cache_Accesses - (AMDStat_L1_Cache_Read_Request +
                                        AMDStat_L1_Cache_Write_Request +
                                        AMDStat_L1_Cache_atomic_w_ret_req +
                                        AMDStat_L1_Cache_atomic_wo_ret_req);
}
...
{
    return AMDStat_L1_Cache_Read_Request +
            AMDStat_L1_Cache_Write_Request +
            AMDStat_L1_Cache_atomic_w_ret_req +
            AMDStat_L1_Cache_atomic_wo_ret_req;
}
...
{
    var hitRate = (AMDStat_TextureCache_Hit / (AMDStat_TextureCache_Hit + AMDStat_TextureCache_Miss) ) * 100;
    return (hitRate > 100 ? 100 : hitRate);
}

function L2CacheMissRate()
{
    var missRate = (AMDStat_TextureCache_Miss / (AMDStat_TextureCache_Hit + AMDStat_TextureCache_Miss)) * 100;
    return (missRate > 100 ? 100 : missRate);
}

function L2CacheHitCount()
{
    return AMDStat_TextureCache_Hit;
}
                    
function L2CacheMissCount()
{
    return AMDStat_TextureCache_Miss;
}
                    
function FetchSize()
{
    return ((AMDStat_L2_Read_Size - AMDStat_L2_Read_Req_32B) * 64) + (AMDStat_L2_Read_Req_32B * 32);
}

function WriteSize()
{
    return ((AMDStat_L2_Write_Size - AMDStat_L2_Write_Req_64B) * 32) + (AMDStat_L2_Write_Req_64B * 64);
}
...
{
    return (AMDStat_CB_L2_Read + AMDStat_CB_L2_Written) / AMDStat_CBDrawn_Busy;
}

function CBMemRead()
{
    return AMDStat_CB_L2_Read * 32;
}

function CBColorAndMaskRead()
{
    return (AMDStat_CB_FC_MC_DCC_Mem_Read + AMDStat_CB_CM_Mem_Read + AMDStat_CB_FC_Mem_Read + AMDStat_CB_L2_Read) * 32;
}

function CBMemWritten()
{
    return AMDStat_CB_L2_Written * 32;
}

function CBColorAndMaskWritten()
{
    return (AMDStat_CB_FC_MC_DCC_Mem_Written + AMDStat_CB_CM_Mem_Written + AMDStat_CB_FC_Mem_Written + AMDStat_CB_L2_Written) * 32;
}
...
function CSVFetchInsts()
{
    if (AMDStat_Compute_Waves_Executed != 0)
        return (AMDStat_Compute_VMEM_RD_Insts - AMDStat_TexUnit_Read_Waves) / AMDStat_Compute_Waves_Executed;
    else
        return 0;
}
...
{
    if (AMDStat_Compute_Waves_Executed != 0)
        return (AMDStat_Compute_VMEM_WR_Insts - AMDStat_TexUnit_Write_Waves) / AMDStat_Compute_Waves_Executed;
    else
        return 0;
}
...
{
    //TODO handle CPF_PERF_SEL_CPF_STAT_BUSY
    return ((AMDStat_Compute_VALU_Active_Cycles / (NumCUPerSH * NumSH)) / AMDStat_GPU_Engine_Busy_Ticks ) * 100;
}
            
function CSVALUBusyCycles()
{
    return AMDStat_Compute_VALU_Active_Cycles / (NumCUPerSH * NumSH);
}

function CSSALUBusy()
{
    //TODO handle CPF_PERF_SEL_CPF_STAT_BUSY
    return ((AMDStat_Compute_SALU_Inst_Cycles / (NumCUPerSH * NumSH)) / AMDStat_GPU_Engine_Busy_Ticks ) * 100;
}

function CSSALUBusyCycles()
{
    return AMDStat_Compute_SALU_Inst_Cycles / (NumCUPerSH * NumSH);
}
...
function CSMemUnitStalled()
{
    ...
    if ( csNumOfThreadGroups != 0)
    {
        var unitStall = ((AMDStat_L1_Stall / AMDStat_GPU_Engine_Busy_Ticks) / NumSE) * 100;
        return (unitStall > 100 ? 100 : unitStall);
    }
    else
        return 0;
...
function CSGDSInsts()
{
 ...
 {
     if (AMDStat_Compute_Waves_Executed != 0)
         return (AMDStat_Compute_LDS_Insts - AMDStat_Compute_FLAT_LDS_Insts) / AMDStat_Compute_Waves_Executed;
     else
         return 0;
 }
...
function ALUutilization()
{
    ...
    util = (DerivedALUThreadCycles/(DerivedALUActiveInst*64)) * 100;
